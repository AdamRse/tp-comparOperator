...
// GETTER BY ID
    ...
    {
        $preparedrequest = $this->db->prepare("SELECT * FROM `destination` WHERE tour_operator_id = ?");
    ...
    {
        $preparedrequest = $this->db->prepare("SELECT * FROM `score` WHERE tour_operator_id = ?");
        $preparedrequest->execute([$tour_operator_id]);
        return $preparedrequest->fetchAll(PDO::FETCH_ASSOC);
    }
    ...
    {
        $preparedrequest = $this->db->prepare("SELECT * FROM `certificate` WHERE tour_operator_id = ?");
        $preparedrequest->execute([$tour_operator_id]);
        return $preparedrequest->fetchAll(PDO::FETCH_ASSOC);
    }
...
// CREATE 
    ...
    {
        $preparedrequest = $this->db->prepare("INSERT INTO review (message, tour_operator_id, author_id) VALUES (:message,:tour_operator_id,:author_id)");
    ...
    {
        $preparedrequest = $this->db->prepare("INSERT INTO review (value, tour_operator_id, author_id) VALUES (:value,:tour_operator_id,:author_id)");
        $request = $preparedrequest->execute($data);
            if ($request) {
                return $this->db->lastInsertId();
            } else 
    ...
    {
        $preparedrequest = $this->db->prepare("INSERT INTO review (name, link) VALUES (:name,:link)");
        $request = $preparedrequest->execute($data);
            if ($request) {
                return $this->db->lastInsertId();
            } else 
