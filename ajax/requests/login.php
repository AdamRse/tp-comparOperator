...
if(isset($_POST['dc'])){//On veut se déconnecter
    ...
    $author = $manager->getAuthorConnect($_POST['name'], $_POST['password']);//On va chercher si c'est un author, revoie la ligne de la bdd si user et mot de passe correspondent
    
    if(empty($author)){//False si l'author a été trouvé, mais que le mdp ne correspond pas
        $tourOperator = $manager->getTourOperatorConnect($_POST['name'], $_POST['password']);
